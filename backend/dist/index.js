...
app.post('/evaluate', (req, res) => {
    ...
    ['firstName', 'lastName', 'email', 'dob'].forEach(field => {
        if (!c[field])
            flags.push({ field, message: `${field} is missing`, severity: 'low' });
    });
    // Legal Status
    ...
    const submissionsPath = path_1.default.join(__dirname, 'submissions.json');
    let submissions = [];
    try {
        const fileData = fs_1.default.readFileSync(submissionsPath, 'utf-8');
    ...
    fs_1.default.writeFileSync(submissionsPath, JSON.stringify(submissions, null, 2));
    res.json({ flags });
});
